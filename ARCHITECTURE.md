...
├── [主线] 输入法激活/停用、状态显示、上屏
├── [音频线] PulseAudio/PipeWire 捕获 → ring buffer → VAD 分段 → 音频缓冲区
...
当前实现优先使用 PulseAudio 后端（通过 `libpulse-simple`，可直接覆盖传统 PulseAudio 和 pipewire-pulse），失败后 fallback 到 PipeWire 直连。PipeWire 直连要求系统中存在 WirePlumber/会话管理器创建的音频节点；只有 PipeWire core 而没有音频节点时会报 `no node available`。
...
├── pulse_audio_capture.cpp # PulseAudio 音频流捕获（优先）
├── pulse_audio_capture.h
├── pipewire_capture.cpp   # PipeWire 音频流捕获（fallback）
├── pipewire_capture.h
...
void VoiceInputEngine::deactivate(
    ...
    // 窗口/InputContext 快速切换时 activate 会取消这次延迟停机。
    scheduleDelayedStopListening();
    activeIc_ = nullptr;
}
...
fcitx5-voice-input
├── fcitx5                      # 输入法框架（核心）
├── pipewire                    # PipeWire fallback 音频捕获
├── libpulse                    # PulseAudio 优先音频捕获
