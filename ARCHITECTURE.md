...
```
Fcitx5 进程 (libvoice-input-addon.so)
├── [主线] 按键事件处理、上屏
├── [音频线] PipeWire 捕获 → ring buffer → VAD 裁剪 → 音频缓冲区
└── [ASR线] sherpa-onnx 推理 → 回调主线程上屏（非阻塞）
```
...
│  │  ┌─────────────────────────────────────────────┐     │   │
│  │  │           主线程 (Fcitx5 事件循环)            │     │   │
│  │  │  ├── Alt_R 按下 → StartRecording()           │     │   │
│  │  │  ├── Alt_R 抬起 → StopRecording() → 投递到 ASR 线程  │     │   │
│  │  │  ├── 结果到达 → commitString()                      │     │   │
│  │  │  └── 状态同步 → 候选词区显示"🎤 录音中..."     │     │   │
│  │  └─────────────────────────────────────────────┘     │   │
│  │                         ↕ 线程安全队列                  │   │
...
| 层级 | 存储方式 | 编辑方式 | 内容 |
|------|---------|---------|------|
| 基本配置 | fcitx5 Addon config | fcitx5-configtool | 按键绑定、模型选择、场景切换 |
...
DefaultValue=
Description=ASR 模型
# 选项在 addon 启动时动态填充：扫描 /usr/share/fcitx5-voice-input/models/*/
# 如果没有安装任何模型包，选项为空，提示用户安装
...
RECORDING
  │  Alt_R 释放 / VAD 超时
  ▼
PROCESSING_ASR ───→ ASR 完成 ───→ 上屏 ───→ IDLE
  │  LLM 后处理（可选）
  ▼
PROCESSING_LLM ───→ 最终结果上屏 → IDLE
```

> ⚠️ 状态切换是瞬时的，主线程不等待 ASR 结果：
> 1. `Alt_R 释放` → 主线程切换 `RECORDING → PROCESSING_ASR`
> 2. 主线程将音频数据投递到 ASR 线程的输入队列 → **立即返回**
> 3. ASR 线程完成推理后，通过 `eventLoop().addDeferredEvent()` 回调主线程
> 4. 回调内主线程执行 `commitString()` → 切换回 `IDLE`
...
void VoiceInputEngine::keyEvent(
    ...
    auto triggerKey = config_.triggerKey();
    
    // 触发键按下 → 开始录音
    if (keyEvent.key() == triggerKey && keyEvent.isRelease() == false) {
        pipeline_.StartRecording();
        keyEvent.filter();  // 吃掉这个按键，不传递给其他程序
        return;
    }
