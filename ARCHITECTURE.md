...
4. **默认云端 ASR** — OpenAI 兼容 API（whisper-1），sherpa-onnx 本地识别可选
...
└── [ASR线] 消费 UtteranceQueue → OpenAI API/sherpa-onnx → ResultQueue →  回调主线程
```
...
│  │  │  ├── 消费 UtteranceQueue                     │     │   │
│  │  │  ├── int16 → float32 转换                   │     │   │
│  │  │  ├── 默认 OpenAI 兼容 API（HTTP multipart）   │     │   │
│  │  │  ├── 可选 sherpa-onnx 本地推理               │     │   │
...
| 键 | 类型 | 默认值 | 说明 |
|----|------|--------|------|
| `ASRBackend` | String | `openai` | ASR 后端 (`openai` / `sherpa`) |
...
public:
    struct Config {
        string modelName;
        string modelPath;      // sherpa-onnx
        int numThreads = 4;
        string apiEndpoint;    // OpenAI
        string apiKey;
        string language = "zh";
    };
...
- 支持 Groq / Together AI / DeepSeek 等兼容服务
- 依赖 libcurl

**SherpaAsrEngine（可选，ENABLE_SHERPA_ONNX=ON）：**
- 使用 sherpa-onnx C API 离线推理
- 内部 `InferenceLoop()` 线程消费 `audioQueue_`
- 在线模式：Start → FeedAudio（多次）→ Stop 触发结果
...
| Sherpa-onnx 未编译 | 运行时检测到 sherpa 配置则报错 |
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `ENABLE_SHERPA_ONNX` | OFF | 启用 sherpa-onnx 本地 ASR |
...
│       │   └── sherpa_asr.cpp/.h # Sherpa-onnx 本地 ASR（可选）
│       └── utils/
...
### Q: 为什么默认用 OpenAI API 而不是 sherpa-onnx？
A: 用户场景以中文语音输入为主，云端 Whisper 在中文准确率上显著优于当前 sherpa-onnx 的 SenseVoice 模型。sherpa-onnx 作为可选离线方案，通过 `-DENABLE_SHERPA_ONNX=ON` 启用。
