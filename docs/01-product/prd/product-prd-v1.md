---
origin_change: product-prd-v1
change_type: feature
cabbage_stage: requirement
synced_at: '2026-09-02T17:31:09.010374+00:00'
---
...
### Scenario 7: 单段识别卡死不阻塞后续语音
- **GIVEN**: 已激活 Voice Input，某一段语音的上游会话无响应（无终态）
- **WHEN**: 用户在等待期间继续说下一段
- **THEN**: 卡住的那段在停滞阈值（40s）后以"语音识别失败"提示放行，后续语音段的结果按说话顺序正常上屏，不需要切走再切回输入法
- [ ] 单段卡死不导致后续语音段结果永久不上屏
...
|---|---|---|---|
| 识别提交文本与发言内容一致性 | 无量化基线（当前无自动化测试） | 人工验收场景 1-7 全通过 | 每次发布验收 |
| 流式后端增量可见 | Volcengine/Mistral/Realtime 支持增量 | 静音判停后增量不丢失、保序上屏；服务端已返回最终结果时立即上屏（不再固定等兜底窗口） | 发布回归窗口 |
| 音频后端回退成功率 | PulseAudio 优先/失败回退已实现 | PipeWire 直连环境正常工作 | 发行版打包 CI |
| 崩溃/异常韧性 | 无已知崩溃 | ASR 错误/空文本/后端缺失不导致 addon 崩溃；单段会话卡死不阻塞后续段（保序闸门停滞放行） | 每次发布回归 |
